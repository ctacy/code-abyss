...
```bash
npx code-abyss-sc
npx code-abyss-sc --list-styles
npm install -g code-abyss-sc@2.0.9
```
...
npx code-abyss-sc --target gemini    # 安装到 ~/.gemini/
npx code-abyss-sc --style abyss-concise --target claude
npx code-abyss-sc --style abyss-concise --target codex
npx code-abyss-sc --target claude -y  # 零配置一键安装 (自动合并推荐配置)
npx code-abyss-sc --target codex -y   # 零配置一键安装 (自动写入 config.toml 模板)
npx code-abyss-sc --target gemini -y  # 零配置一键安装 (自动生成 GEMINI.md + TOML commands)
npx code-abyss-sc --list-styles       # 列出可用输出风格
npx code-abyss-sc --uninstall claude  # 卸载 Claude Code
npx code-abyss-sc --uninstall codex   # 卸载 Codex CLI
npx code-abyss-sc --uninstall gemini  # 卸载 Gemini CLI
```

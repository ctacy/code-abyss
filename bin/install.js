...
function maybeInstallEnforcement(targetName, ctx) {
  ...
  const { spawnSync } = require('child_process');
  // stdio: ['ignore', 'pipe', 'pipe'] — bash must NOT inherit the parent's
  // stdin pipe from a piped/TUI-context parent process (Windows + Git Bash
  // blocks indefinitely on inherited open pipes; harmless on Unix but
  // strictly safer to drop the inheritance either way).

...
> **Product code today:** v5.0.0-rc.2 (`package.json`). This document is design-only; **today** vs **target** are labeled everywhere they diverge. The v4.10-era "today" narrative below is preserved as historical context; v5.0.0-rc has landed the kill-foyer, runtime control plane (`doctor`/`compose`/`score`), default enforcement, and inject plane.
...
## 1. Diagnosis — today (v5.0.0-rc tree facts; v4.10 narrative preserved below)
